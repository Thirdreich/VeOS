...
namespace VlaicuOS.Graphics
{
    public class GUI
    {
        ...
        public void handleGUIInputs ()
        {
            ...
            {
                //Если код здесь работает, значит, мышь переместилась с момента последнего кадра.
                ...
                if (MouseManager.X < 2 || MouseManager.Y < 2||MouseManager.X>(MouseManager.ScreenWidth=2)||MouseManager.Y>(MouseManager.ScreenHeight=2))
                    // Если мышь пытается выйти за пределы, не двигайте мышь!!!
                    return;

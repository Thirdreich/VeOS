...
function tip_out()
{
	if (cw_l.length > 0)
	{
		document.write(tl_9, ' ', cw_l, '');
	
	}
	if (sp_l.length > 0)
	{
		sp_e = encodeURIComponent(sp_l);
		document.write(tl_8, ' <a href="', r_l, '?d=', sp_e, '">', sp_l, '</a>');	
	}
	if (co == 0)
	{
		if (v_d)
		...
		else
		{
			...
			// Supa dupa hack for Spotlight appearence
			// by SE51, Build [1M0104]
			
			else document.write('<table width=100% border=0 cellspacing=0 cellpadding=0 align=left><tr><td align=center valign=top bgcolor=#f1f1f1><font face=\'Lucida Grande, Tahoma\' size=1 color=#666666><b>' + od + '</b><br><br><br></font></td></tr></table>');
	...
	for (a = tn; a < nd; a++)
	{
		...
		if (os[3].length > 1) document.write('', os[3]);
		if (include_url == 1) document.write('<a href="', os[2], '" ', t_b, '>', os[2], '</a>');
		document.write('');
	}
}
